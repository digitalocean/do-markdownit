...
module.exports = (md, options) => {
    ...
    const calloutRule = (state, startLine, endLine, silent) => {
        ...
        if (optsObj.allowedClasses && !optsObj.allowedClasses.includes(className)) return false;
        ...
        // Check for a label
        let workingStartLint = startLine;
        ...
        if (labelMatch) {
            ...
            labelOpenToken.map = [ workingStartLint + 1, workingStartLint + 2 ];

            // Create an inline token for the content
            const labelToken = state.push('inline', '', 0);
            labelToken.content = labelMatch[1];
            labelToken.map = [ workingStartLint + 1, workingStartLint + 2 ];
            labelToken.children = [];
        ...
        state.md.block.tokenize(state, workingStartLint, state.line);
